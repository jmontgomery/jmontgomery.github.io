...
## Beyond artificial meth problems
...
What we want to do (first) is construct a confidence interval for
...
= 0.067$$ \pause $$z = \displaystyle \frac{(\bar{y_2} - \bar{y_1}) - 0}{\hat{\sigma}_{\bar{y_2}
  - \bar{y_1}} } = \pause \frac{0.515-0.435}{0.067} = \pause 1.193 $$
  

Interpret this? 

\texttt{pnorm(1.194, lower.tail=FALSE)}=0.116
...
- $\bar{y_2} - \bar{y_1} = 69.62 - 664.77 = 4.85$ \pause
- $\hat{\sigma} = \sqrt{\frac{(29-1)3.19^2 + (24-1)3.84^2}{29+24-2}} = 3.56$  \pause
- $\hat{\sigma}_{\bar{y_2} - \bar{y_1}} =  \hat{\sigma}\sqrt{\frac{1}{n_1} + \frac{1}{n_2}}=3.56 \sqrt{\frac{1}{24} + \frac{1}{29}} =0.982$  \pause
- $d.f. = 24+29-2 = 51$ \pause
- $t_{51, .05} =$ \texttt{qt(.025, df = 51, lower.tail = FALSE)} $= 2.01$ 

\pause

95\% CI: $4.85 \pm \big(2.01 \times 0.982) = (2.88, 6.62)$ \pause Interpret? 

Hypothesis test: $t=  \frac{4.85 - 0}{0.982} \approx 4.94$  \pause  $\mathtt{2*pt(4.94, df=51, lower.tail=F)} \approx 0$
