...
- For this class we'll use something maximum likelihood estimation, that is way outside the scope of this class.
...
- All you really need to know is how to do it.
    ...
    * We specify tht the data is binary by using the `family="binomial"` argument.
...
Let's look at the predictd probabilities for each value of Age in the dataset.
...
\text{Prediction = 1} & \text{False negatives} & \text{True Positives}\\
\end{array}$$
...
- Should you weight positives or negatives more?
    ...
    - You will want to ballance or weight.
    
---

- One appproach is to look at precision and recall
    - **Precision** $$\frac{\text{True positives}}{\text{True positives + Fase posiives}}$$
    - **Recall** $$\frac{\text{True positives}}{\text{True positives + Fase negatives}}$$

- You can also recombine these in a bunch of ways to get things like false discoery rate, specificity, sensitivity, F1 scores, and more.
...
- Most often, these parameters control how *complex* each model is going to be.
- In the case of the `rpart` function these include:
...
table(turnout$turnout, mod1_knn)
table(turnout$turnout)
```
