...
1. Advnanced text topics (`stringr`)
...
- You are going to want to use escapes.  But because `\` is itself part of the regular expressions syntax you need to double escape `\\`.  So to find `.` you need to search for `\\.`  (And you will need to use `\\\\` to searc for `\`)
...
1. Count the number of times that Krewson tweets included the string 'polic'. Count the number of times that Krewson tweets the *word* 'police' (See 14.3.2).
2. Count the numer of times her twweets include a link (look at http)
3. Subset to tweets that include the string "Black Lives".  See section 14.5 in the book to see how to do this while ignoring the case.
