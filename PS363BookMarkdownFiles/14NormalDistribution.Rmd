...
The probability $P(X<x)$ for a normal probability distribution in R is given by the <code>pnorm()</code> function.  So assume we have a normal distribution with $\mu=5$, and $\sigma=2$. The are to the left $X=6$ for the normal curve is found by coding:
...
We can then look up the height of the *standard normal* distribution
```{r}
pnorm(.5, mean=0, sd=1)
```
